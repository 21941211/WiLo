...
void SM_Measure()
{
  ...
  float vol_water_cont; // preallocate to approx. voltage and theta_v
  uint16_t adc;
  ...
  if(SM_Percentage<=0){
    ...
    Serial.print(wilo.SM); // read sensor
    Serial.println(" mV");
    ...
    vol_water_cont = ((1.0 / (wilo.SM / 1000.0)) * slope) + intercept; // calc of theta_v (vol. water content)
