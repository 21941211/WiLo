...
#include "DeepSleep_Driver.h"
 ...
 RTC_DATA_ATTR uint8_t SDI12_TYPE = 0;
...
void setSDI12DummyValues(uint8_t sdi12Type){
  Serial.println("%-----------------------------------------------------------%");
  Serial.println("Setting dummy values for SDI12");
  switch (sdi12Type)
  {
  case SDI12_DD_60:
    SDI12_TYPE = SDI12_DD_60;
  ...
  for (int i=0; i<bufferSizeSDI12; i++){
    SDI12_Temp.push_back(24.40);
    SDI12_SM.push_back(20.00);
  }

  for (int i=0; i<bufferSizeSDI12; i++){
    Serial.println("Temperature: "+String(SDI12_Temp[i]));
    Serial.println("Soil moisuture: "+String(SDI12_Temp[i]));
  }
...
uint8_t getBuffSizeSDI12(){
  return bufferSizeSDI12;
}